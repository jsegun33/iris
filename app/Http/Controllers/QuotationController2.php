    ...
    foreach($RequestCoverages as $RequestCoveragess){ 
      ...
      if ( $RequestCoveragess->PerilsCode == 'AOG'  ){   
            ...
            $WithAOG = "NO";
              ...
              //$RequestCoveragess->Deductible           = round($DeductableNew , 2);
...
//-----------------Coverages------------------------------------------>		
          ...
          //$GetOption          = RequestCoverages::where('_id',$CoveragesNameID1)->first(); 
          for($kk=1 ;$kk < $RequestNoOptionNo   ;$kk++)
          {  
                 // $RequestCoveragesss   = RequestCoverages::where('RequestNo',$GetOption->RequestNo)->where('OptionNo',$GetOption->OptionNo)->get();
                 $CoveragesNameID1            =  trim($request['PerilsNameForDisplay'][$kk]);
                 $RequestCoveragess          = RequestCoverages::where('_id',$CoveragesNameID1)->first(); 

            $RequestCoveragesss   = RequestCoverages::where('RequestNo',$request['RequestNoPass'])->where('OptionNo',$kk)->get();
              ...
              $RequestCoveragesUpdate1   = RequestCoverages::where('RequestNo',$request['RequestNoPass'])->where('OptionNo',$kk)->get();

              
              foreach($RequestCoveragesUpdate1 as $RequestCoveragesUpdate){ 
                    
                  $RequestCoveragesAOG1    = RequestCoverages::where('RequestNo',$request['RequestNoPass'])->where('OptionNo',$kk)->where('PerilsCode','AOG')->first();
              ...
              $RequestCoveragesOD    = RequestCoverages::where('RequestNo',$request['RequestNoPass'])->where('OptionNo',$kk)->where('PerilsCode','OD')->first();
              $RequestCoveragesTF    = RequestCoverages::where('RequestNo',$request['RequestNoPass'])->where('OptionNo',$kk)->where('PerilsCode','TF')->first();
              $CAmountODTF   = $RequestCoveragesOD->CoveragesAmount   +  $RequestCoveragesTF->CoveragesAmount;
              $PAmountODTF   = $RequestCoveragesOD->CoveragesPremium  +  $RequestCoveragesTF->CoveragesPremium; 
              ...
              $RequestCoveragesTF->save(); 
                    ...
                    //Deductable
                    $GetDenomination     = RequestDetails::where('RequestNo',$request['RequestNoPass'])->first();
                    $GetDenominationExplode  =  explode('-' ,$GetDenomination->Denomination);
                    ...
                    if (   $GetDenomination->WithAOG !== "YES" ) {
                      $Deductable =  $PAmountODTF1  * $FindDefaultDeductible->Amount; 
                      if ($Deductable < $FindDefaultDeductible->MinAmount){  //less than the declare amount
                          $DeductableNew             =  $FindDefaultDeductible->MinAmount + 2;
                      }else{
                    ...
                    $CoveragesDeductible   = RequestCoverages::where('RequestNo',$request['RequestNoPass'])->where('OptionNo',$kk)->get();
                    foreach($CoveragesDeductible as $CoveragesDeductibles){ 
                        $CoveragesDeductibles->Deductible           = round($DeductableNew , 2);
                        $CoveragesDeductibles->save(); 
                    }
	...
	//-----------------Coverages------------------------------------------>		
      ...
      //------------Charges------Charges-------Charges----->   
          ...
          $ChargesForDisplay        = $request['TxtChargesNameID'];
               ...
               {   
                     ...
                     // $RequestCharges->ChargesPremiumAOG    = $CompChargesAmount1;
					  $RequestCharges->save();
					 
           
               }

				$RequestCoveragesAOG    = RequestCoverages::where('RequestNo',trim($request['RequestNoPass']))->where('OptionNo',$kk)->where('PerilsCode','AOG')->first();
	 ...
	 $FindProductLineCharge1  = ProductLinesCharges::select('*')->where('ProductLine','2019-MC-0001')->where('Active','1')->get();
              ...
              { 
                  ...
                  $CompCharges1 += $CompChargesAmount1; 
                  $UpdateCharges2    = RequestCharges::where('RequestNo',trim($request['RequestNoPass']))->where('OptionNo',$kk)->where('ChargesNo',$FindProductLineChargess->ChargesNo)->get();
                  foreach($UpdateCharges2 as $UpdateCharges22){ 
          ...
          $UpdateCharges2    = RequestCharges::where('RequestNo',trim($request['RequestNoPass']))->where('OptionNo',$kk)->get();
          $TotalCharges = 0 ;
...
public function  ListCoveragesForApproval($id, request $request)
 ...
 public function  PolicyModificationEditBtn($id, request $request)
  ...
  //Deductable
          $GetDenomination     = RequestDetails::where('RequestNo',trim($id))->first();
          $GetDenominationExplode  =  explode('-' ,$GetDenomination->Denomination);
