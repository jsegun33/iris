     ...
     <section class="content" @mouseover="VerifyAccessRoles()"    v-if="this.RequestQuotations !== 'NO RECORD FOUND' && this.AllRolesList ==='YES ACCESS'">
               ...
               <h1> UNPAID Policy /ies</h1>
                    ...
                    <div slot="actions" slot-scope="{ row }">
                        ...
                        <a :href="'/Payment-Mode-Cashier?' + row.RequestNo"  v-if="row.PaymentMode !== 'Paid' && row.PaymentGateway  === 'Cashier'"  class="btn btn-warning" style="text-decoration: none;"> <i class="fa fa-money"></i> Pay Now</a>
     ...
     mounted: function() {
        ...
        methods: {
          ...
          async  loadRequestQuotation() {
                //         axios.get("api/UnpaidPolicies").then(({ data }) => (this.RequestQuotations = data));
                ...
                //     },1000) 
                     try {
                         await   axios.get("api/UnpaidPolicies").then(({ data }) => (this.RequestQuotations = data));
                    } catch(error) {
